...
```{r, include=F}
influence_variable_name <- tibble(variable_name = c("PHTYPE", "INPATIENT", "TOTCDDX", "COGNFUNC", "TOTALADL"))

influence_variable_coefficient <- tibble(relative_influence = c(56.45, 7.50, 6.18, 5.47, 4.84))

influence <- cbind(influence_variable_name, influence_variable_coefficient)
```
...
Model <- tibble(model = c("multivariable_fit_all", "multivariable_fit_subset", "tree", "bag", "random_forest", "boost", "neural_network"))
MSE <- tibble(mse = c(21953.98, 21841.16, 21212.89, 24476.81, 23296.75, 22150.37, 11089.06))
error <- cbind(Model, MSE)
error <- error %>% mutate(
  rmse = sqrt(mse)
)
```
